...
If a "Market context" block is present, treat it as ADVISORY macro regime (not a hard gate): broad-red 6h+24h, weekend, or net-buyer breadth divergence (5m breadth meaningfully below 6h, e.g. Δ ≤ −15pp = leaders losing short-term flow → exhaustion/rotation) warrants tighter entry confidence and faster exit on weakening setups. Note: trending-cohort 6h breadth is structurally near 100% (selection bias), so the 5m vs 6h delta is the actionable signal — not the absolute level. Do not veto on macro alone if per-token signals are strong. If a "Bot recent performance" block is present, treat it as ADVISORY: a losing streak (≥3) warrants tighter entry confidence and bias toward skip on borderline; a winning streak is not license to relax thresholds. Sample is small — do not overweight.`;
